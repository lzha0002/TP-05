...
if (!class_exists('AdvancediFramePrepareJs')) {
    ...
    static function aiPrepareAiSrcUrl(&$src, $map_parameter_to_url, $add_iframe_url_as_param_prefix, $url_forward_parameter, $pass_id_by_url, $id, $src_orig) {
		// keep ? and & encoded!
				 ...
				 if (count($parameter_url_mapping) === 3) {
					...
					if (empty($src_url) && $prefix === "hashrewrite") {
						...
						if (AdvancedIframeHelper::aiContains ($fullurl, '/' . $iframeParam . '/' )) { 
							$src_url = end(explode('/', AdvancedIframeHelper::aiRemoveQueryString($fullurl)));	
						}
					}
	...
	static function aiPrepareChangeCss(&$hideiframehtml, $id, $iframe_content_id, $iframe_content_styles) {
	...
	static function aiPrepareChangeIframesLinks(&$hideiframehtml, $change_iframe_links, $change_iframe_links_target, $change_iframe_links_href, $id) {
...
$error_html = AdvancediFramePrepareJs::aiPrepareAiSrcUrl($src, $map_parameter_to_url, $add_iframe_url_as_param_prefix, $url_forward_parameter, $pass_id_by_url, $id, $src_orig);
if ($error_html) {
	return $error_html;
}
...
$error_html = AdvancediFramePrepareJs::aiPrepareChangeCss($hideiframehtml, $id, $iframe_content_id, $iframe_content_styles);
if ($error_html) {
	return $error_html;
}

$error_html = AdvancediFramePrepareJs::aiPrepareChangeIframesLinks($hideiframehtml, $change_iframe_links, $change_iframe_links_target, $change_iframe_links_href, $id);
if ($error_html) {
	return $error_html;
}
