...
?>    
	 <p><?php _e('The external workaround does enable many features which are not possible directly because of cross domain security restrictions. You need to include a Javascript file (ai_exernal.js) to the page in the iframe which is generated dynamically with your settings. <strong>If you mix http and https pages you NEED to enable "<a class="link-id-external-ai-config-post" href="#upm">Use postMessage for communication</a>" and follow  <a href="//www.tinywebgallery.com/blog/advanced-iframe/advanced-iframe-pro-demo/external-workaround-with-post-message#e53" target="_blank">example 53</a> for this advanced setup (pro version needed)</strong>! In the advanced tab are many settings marked with ', 'advanced-iframe');
    echo renderExternalWorkaroundIcon(true);
    _e('. This means that this setting is saved to the ai_exernal.js.', 'advanced-iframe') ?>
    </p>
	<p><?php _e('Please read the <a href="//www.tinywebgallery.com/blog/advanced-iframe-resize-to-content-summary" target="_blank">Resize to content summary</a> first if you want to use auto height.', 'advanced-iframe') ?>        
    </p>
...
<?php	
	aiPostboxClose();
    aiPostboxOpen("id-external-subdomain", 'Special case sub domain', $closedArray); 
?>  	
<p><?php _e('If your iframe has the same second level domain an easier way is possible. You still have to include one line of Javascript but the whole configuration is than like you are on the same domain and all settings can be done by shortcode. To enable this you need to set the value "document.domain" to your second level domain (e.g. parent: www.example.com, iframe: subdomain.example.com -> second level domain: example.com) with Javascript on both pages. The plugin does set this value to the domain below if you enable this feature. See <a href="//www.tinywebgallery.com/blog/using-sub-domains-with-advanced-iframe" target="_blank">this blog entry</a> or <a href="//www.tinywebgallery.com/blog/advanced-iframe/advanced-iframe-pro-demo/sub-domain-auto-height-and-css-modifications" target="_blank">example 42</a> for further details.
</p><p>
...
<?php
aiPostboxClose();
aiPostboxOpen("id-external-different", "If you are on a different domain (not sub domain)", $closedArray); 
      ...
      if ( $channel === 'iframe' && $evanto) {
		echo '<div class="manage-menus nounderline sub-domain-container hide-search" style="color:red;height:20px;">';  
...
<?php _e('<p>If it does not work please check the following things:</p>
	    ...
	    _e('">here</a> and also force a full reload. Sometimes the file is cached by a proxy. Check the time stamp (Created:) on the top of the file.</li>
           ...
           <li>Make sure the ids do match! Often the demos are copied! so the shortcode has a id from the example but the ai_external.js has the default from the basic tab. Make sure to use the same id for both!
           <li>If you use iframe communication check the network tab (F12), filter for doc/html and look for height.html. There you should see the measured height as parameter. If this does not show switch to <a href="#upm" class="link-id-external-ai-config-post">postMessage communication</a> as this way has less restrictions.</li>
...
<?php
  ...
  _e('<p>
    ...
    echo renderExternalWorkaroundIcon(true);
    _e(' is shown! You only need to define variables before the script or a configuration file if you need to include the SAME ai_external.js with DIFFERENT configurations. See the <a href="//www.tinywebgallery.com/blog/advanced-iframe/advanced-iframe-faq#config" target="_blank">FAQ</a> for more details.
    </div>
