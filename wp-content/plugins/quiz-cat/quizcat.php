...
if ( !defined ('FCA_QC_PLUGIN_DIR') ) {
	...
	if ( FCA_QC_DEBUG ) {
		define( 'FCA_QC_PLUGIN_VER', '2.0.' . time() );
	} else {
		define( 'FCA_QC_PLUGIN_VER', '2.0.6' );
	}
	...
	function fca_qc_admin_deactivation_survey( $hook ) {
		if ( $hook === 'plugins.php' ) {
			...
			<div id="fca-deactivate" style="position: fixed; left: 232px; top: 191px; border: 1px solid #979797; background-color: white; z-index: 9999; padding: 12px; max-width: 669px;">
				...
				<p><b><?php _e( 'If you\'re upgrading to Quiz Cat Premium and have questions or need help, click <a href=' . 'https://fatcatapps.com/article-categories/gen-getting-started/' . ' target="_blank">here</a></b>', 'quiz-cat' ) ?>
				</p>
