...
class Assets {
	...
	public function load_admin_scripts() {
		...
		if ( $typenow === 'igmap' && ( $pagenow === 'post.php' || $pagenow === 'post-new.php' ) ) {
			...
			$meta['admin']                        = '1';
			$meta['container_info']               = 'map_tech_info';
			...
			wp_register_script(
				...
				plugins_url( 'assets/js/admin-scripts.js', $this->core->file_path ),
				array(),
				$this->core->version,
				true
			);
			...
			$this->options['messages']       = [
				...
				'maxInputError' => __( '<strong>Attention!</strong> Your map has too many entries for your server to handle when saving it. <a href="https://interactivegeomaps.com/docs/max-input-vars-issue" target="_blank">Read more about this issue here</a>.', 'interactive-geo-maps' ),
	...
	public function load_json_tree() {
		wp_register_script(
			...
			plugins_url( 'assets/vendor/jsonTree/jsonTree.js', $this->core->file_path ),
			array(),
			$this->core->version,
			true
		);
		...
		wp_register_style(
			$this->core->name . '_jsontree',
			plugins_url( 'assets/vendor/jsonTree/jsonTree.css', $this->core->file_path ),
			false,
			$this->core->version
		);
	...
	public function load_autocomplete() {
		wp_register_script(
			...
			plugins_url( 'assets/vendor/autocomplete/autocomplete.js', $this->core->file_path ),
			array(),
			$this->core->version,
			true
		);
		...
		wp_register_style(
			$this->core->name . '_autocomplete',
			plugins_url( 'assets/vendor/autocomplete/autocomplete.css', $this->core->file_path ),
			false,
			$this->core->version
		);
	...
	public function load_map_styles() {
			...
			wp_register_style(
				...
				plugins_url( 'assets/css/styles.css', $this->core->file_path ),
				false,
				$this->core->version
			);
	...
	public function load_map_scripts( $localize = false ) {
		...
		$version = 'version/4.9.30';

		if ( isset( $options['amcharts_url'] ) && trim( $options['amcharts_url'] ) !== '' ) {
		...
		wp_register_script(
			...
			plugins_url( 'assets/map-service/app.min.js', $this->core->file_path ),
			$deps,
			$this->core->version,
			true
		);
