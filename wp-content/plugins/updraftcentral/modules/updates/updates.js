	...
	function send_remote_updates(site, update_info) {
		...
		UpdraftCentral.send_site_rpc('updates.do_updates', uc_updates, $site_row, function(response, code, error_code) {
			...
			if ('ok' == code && response) {
				...
				if (any_successes) {
					update_cache_entry_after_updates($site_row).then(function(data) {
						if (debug_level > 0) {
							console.log('send_remote_updates; cache entry updated for the given site: '+$site_row.data('site_description'));
							console.log(data);
						}
					});
				}
